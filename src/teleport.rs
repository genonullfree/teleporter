...
impl TeleportDelta {
    ...
    pub fn serialize(self) -> Vec<u8> {
        ...
        let dlen = self.delta_hash.len() as u16;
        out.append(&mut dlen.to_le_bytes().to_vec());

        // Add delta vector
        out.append(&mut TeleportDelta::delta_serial(&self.delta_hash));
    ...
    pub fn deserialize(&mut self, input: &[u8]) -> Result<(), Error> {
        ...
        self.delta_hash_len = buf.read_u16::<LittleEndian>().unwrap();

        // Extract delta vector
        self.delta_hash = TeleportDelta::delta_deserial(buf, self.delta_hash_len).unwrap();
